...
def asset_json(p):
    ...
    history = AssignmentHistory.query.filter_by(personnel_id=p.id).order_by(AssignmentHistory.id.desc()).all()

    latest_license_ids = []
    seen = set()
    for h in history:
        if h.asset_type == "license" and h.asset_id not in seen and h.action == "assign":
            seen.add(h.asset_id)
            latest_license_ids.append(h.asset_id)
    licenses = License.query.filter(License.id.in_(latest_license_ids)).all() if latest_license_ids else []
    license_map = {x.id: x for x in licenses}
    licenses = [license_map[x] for x in latest_license_ids if x in license_map]

    stock = StockMovement.query.filter_by(personnel_id=p.id).order_by(StockMovement.id.desc()).all()
    return {
        "inventory": [{
            "id": x.id, "inventory_no": x.inventory_no, "computer_name": x.computer_name,
            ...
            "model": x.model.name if x.model else None,
        } for x in inventory],
        "licenses": [{
            "id": x.id, "name": x.license_name.name if x.license_name else None,
            "expires_at": x.expires_at.isoformat() if x.expires_at else None,
        } for x in licenses],
        "stock_movements": [{
            "id": x.id, "stock_item_id": x.stock_item_id, "movement_type": x.movement_type,
...
def audit(action, entity_id, details=None):
    ...
    db.session.add(AuditLog(
        action=action, entity_type="personnel", entity_id=entity_id,
...
def get_personnel(person_id):
    ...
    data["history"] = [{
        "id": h.id, "asset_type": h.asset_type, "asset_id": h.asset_id, "action": h.action,
        "note": h.note, "created_at": h.created_at.isoformat(),
    } for h in AssignmentHistory.query.filter_by(personnel_id=p.id).order_by(AssignmentHistory.id.desc()).all()]
    return jsonify(data)
