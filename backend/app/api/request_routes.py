...
from ..models import AuditLog, Department, Factory, Personnel, ProductType, PurchaseRequest, PurchaseRequestItem

requests_bp = Blueprint("requests", __name__)

STATUSES = {"draft", "pending", "approved", "rejected", "ordered", "completed", "cancelled"}
...
def _audit(action, entity_id, details=None):
    db.session.add(AuditLog(action=action, entity_type="purchase_request", entity_id=entity_id, details=details or {}))

def _payload(data, existing=None):
    no = str(data.get("request_no", existing.request_no if existing else "")).strip()
    if not no: raise ValueError("Talep numarası zorunludur")
    status = str(data.get("status", existing.status if existing else "pending")).strip().lower()
    priority = str(data.get("priority", existing.priority if existing else "normal")).strip().lower()
    ...
    requester = _resolve(Personnel, data.get("requester_id", existing.requester_id if existing else None), "talep sahibi")
    department = _resolve(Department, data.get("department_id", existing.department_id if existing else None), "departman")
    ...
    if not isinstance(raw_items, list) or not raw_items: raise ValueError("En az bir talep kalemi eklenmelidir")
    items=[]
    for raw in raw_items:
        product_type=str(raw.get("product_type", "")).strip()
        if not product_type: raise ValueError("Ürün tipi zorunludur")
        try: quantity=float(raw.get("quantity", 1))
        except (TypeError, ValueError): raise ValueError("Miktar geçersiz")
        if quantity <= 0: raise ValueError("Miktar 0'dan büyük olmalıdır")
        price=raw.get("estimated_unit_price")
        if price not in (None, ""):
            try: price=float(price)
            except (TypeError, ValueError): raise ValueError("Tahmini birim fiyat geçersiz")
        items.append(PurchaseRequestItem(product_type=product_type,device_type=raw.get("device_type") or None,brand=raw.get("brand") or None,model=raw.get("model") or None,quantity=quantity,unit=raw.get("unit") or "Adet",estimated_unit_price=price,description=raw.get("description") or None))
...
def list_requests():
    ...
    search=request.args.get("search","").strip(); status=request.args.get("status","").strip(); priority=request.args.get("priority","").strip()
    if search:
    ...
    page=max(request.args.get("page",1,type=int),1); per_page=min(max(request.args.get("per_page",25,type=int),1),100); p=q.order_by(PurchaseRequest.id.desc()).paginate(page=page,per_page=per_page,error_out=False)
...
def get_request(request_id):
    ...
    return jsonify(_dict(x)) if x else (jsonify({"error":"Talep bulunamadı"}),404)

@requests_bp.post("/requests")
def create_request():
    try:
        data=_payload(request.get_json(silent=True) or {})
        items=data.pop("items"); x=PurchaseRequest(**data); x.items=items; db.session.add(x); db.session.flush(); _audit("request.created",x.id,{"request_no":x.request_no,"item_count":len(items)}); db.session.commit(); return jsonify(_dict(x)),201
...
def _set_status(request_id,status):
    ...
    if status=="approved": x.approved_at=datetime.now(timezone.utc); x.approved_by=data.get("approved_by") or "Sistem"
