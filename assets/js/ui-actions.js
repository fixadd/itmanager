document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.addEventListener('click',e=>{
    ...
    if(/Yeni Envanter|Yeni Lisans|Stok Girişi|Stok Çıkışı|Bakım Kaydı|Servise Gönder|Yeni Talep|Yeni Satın Alma Talebi|Yeni Personel|Yeni Makale|Kullanıcı Ekle|Yeni Kayıt/i.test(t)){
      e.preventDefault();e.stopImmediatePropagation();
      const title=t||'Yeni Kayıt';
      const p=page();
      const labels=p.includes('inventory')?['Envanter No','Cihaz Tipi','Marka','Model','Seri No','Personel','Lokasyon','Durum']:p.includes('licenses')?['Yazılım','Lisans Tipi','Firma','Lisans Anahtarı','Kapasite','Bitiş Tarihi','Durum']:p.includes('stock')?['Stok No','Ürün','Kategori','Miktar','Birim','Kritik Seviye','Lokasyon','Durum']:p.includes('maintenance')?['Envanter No','Personel','Arıza','Teknisyen','Başlangıç','Sonuç','Durum']:p.includes('requests')?['Talep No','Talep Türü','Talep Eden','Departman','Fabrika','Ürün / Hizmet','Miktar','Öncelik','Açıklama','Durum']:p.includes('people')?['Ad Soyad','Sicil No','Departman','Fabrika','Pozisyon','Durum']:['Başlık','Kategori','Açıklama','Durum'];
      const fields=labels.map(x=>({label:x,value:x==='Durum'?'Yeni':'—'}));
      if(window.ITUI){ const form='<div class="row g-3">'+labels.map(x=>'<div class="col-md-6"><label class="form-label">'+esc(x)+'</label>'+(x==='Durum'?'<select class="form-select"><option>Yeni</option><option>Onay Bekliyor</option><option>Onaylandı</option><option>Satın Alma Bekliyor</option><option>Ürün Bekleniyor</option><option>Tamamlandı</option><option>Reddedildi</option></select>':x==='Talep Türü'?'<select class="form-select"><option>Envanter</option><option>Lisans</option><option>Stok / Sarf</option></select>':x==='Öncelik'?'<select class="form-select"><option>Düşük</option><option selected>Normal</option><option>Yüksek</option><option>Acil</option></select>':'<input class="form-control" placeholder="'+esc(x)+'">')+'</div>').join('')+'</div>'; ITUI.modal(title,form,{size:p.includes('inventory')?'modal-xl':'modal-lg'}); } else { modal(title,fields,[{id:'save',text:'Kaydet',icon:'device-floppy',class:'btn-primary'},{id:'cancel',text:'Vazgeç',icon:'x',class:'btn-light'}]); }
    }
  },true);
});
